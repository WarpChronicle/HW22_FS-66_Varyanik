...
acc1.deposit(300);       
acc2.withdraw(700); // списываю больше чем есть чтобы проверить
...
function transfer(from, to, amount) {
  if (amount > 0 && typeof amount === 'number' && from.balance >= amount) {
...
function transfer(from, to, amount) {
  ...
  if (amount > 0 && typeof amount === 'number' && from.balance >= amount) {
    ...
    transaction.transInfo = function() {
      console.log(`Успешный перевод: на сумму ${this.amount}$ с ${this.acc1.iban} на ${this.acc2.iban}`);
    };
  } else {
